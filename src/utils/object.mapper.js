...
import {html} from "core/i18n"
import {safeAtob} from 'utils/base64'
import {getJobUpdateTime, getWorkloadUpdateTime} from 'utils/workload'
    ...
    if (!labels['iam.kubesphere.io/role-template']) {
        switch (module) {
            case 'workspaceroles': {
                ...
                if (window.globals.config.presetWorkspaceRoles.includes(name)) {
            ...
            case 'globalroles': {
                ...
                if (window.globals.config.presetGlobalRoles.includes(name)) {
                    baseInfo.description = (
            ...
            case 'clusterroles': {
                const name = baseInfo.name
                if (window.globals.config.presetClusterRoles.includes(name)) {
                    baseInfo.description = (
                        `ROLE_${name.toUpperCase().replace(/-/g, '_')}`
                    )
                }
            ...
            case 'roles': {
                const name = baseInfo.name
                if (window.globals.config.presetRoles.includes(name)) {
                    baseInfo.description = (
...
const ClusterMapper = item => {
    ...
    return {
        ...
        isReady: window.globals.app.isMultiCluster
