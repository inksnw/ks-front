...
export const getPipelineStatus = statusDetail => {
  ...
  if (state === 'QUEUED') {
    return { type: 'queued', label: ('QUEUED') }
  }
  if (state === 'RUNNING') {
  ...
  if (state === 'FINISHED' && result === 'SUCCESS') {
    return { type: 'success', label: ('SUCCESSFUL') }
  }
  if (state === 'FINISHED' && result === 'FAILURE') {
    return { type: 'failure', label: ('FAILED') }
  }
  if (state === 'FINISHED' && result === 'ABORTED') {
    return { type: 'aborted', label: ('ABORTED') }
  }
  if (state === 'PAUSED') {
    return { type: 'paused', label: ('PAUSED') }
  }
  return { type: 'nostatus', label: ('NOT_RUNNING') }
}
